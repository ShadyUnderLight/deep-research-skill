...
def run_delivery(
    ...
    *,
    ...
    result = DeliveryResult(input_path=input_path, pdf_path=pdf_path, kept_html=keep_html)
    ...
    with tempfile.TemporaryDirectory(prefix="deep-research-delivery-") as temp_dir:
        ...
        try:
            ...
            result.pdf_size_bytes = pdf_path.stat().st_size if pdf_path.is_file() else None
        except Exception as exc:
            result.delivery_status = DeliveryStatus.PDF_FAILED
            result.errors.append(f"HTML to PDF failed: {exc}")
