...
## Benchmark comparability for academic reviews

When an academic / literature review report's conclusions depend on benchmark number comparisons — model scores on evaluation benchmarks, benchmark performance comparisons across papers, or quantitative capability claims — the report must disclose sufficient methodology context for the reader to assess comparability.

This discipline extends the benchmark comparability rules from `references/technical-analysis-discipline.md` §Benchmark comparability for technical deep-dive (which focus on engineering / deployment context) to the academic-review context, where the relevant disclosure fields differ.

### 触发条件

Benchmark comparability schema is required when the report contains any of the following:

- **多模型 benchmark 对比**：在同一分析中比较两个或以上模型的 benchmark 分数
- **能力领先声称**：声称某模型在某能力（如规划、推理、视觉理解）上显著领先
- **benchmark 分数支撑"真实推理能力"**：用 benchmark 结果推断模型的"真实推理能力"或"通用智能"
- **讨论 leakage / shortcut / contamination / no-image baseline**：涉及 benchmark 有效性或方法学争议

### Minimum disclosure fields for academic AI benchmarks

For each load-bearing benchmark comparison, the report must disclose the following fields. Mark genuinely inapplicable fields as `N/A` rather than omitting them silently.

| 字段 | 含义 | 示例 |
|------|------|------|
| **Benchmark / Task** | 具体 benchmark 名称及任务变体 | GSM8K (数学推理), MMLU (知识), HumanEval (代码) |
| **Capability claim** | 声称支撑的能力类型 | 数学推理 / 规划 / 多步推理 / 视觉理解 |
| **Model / version** | 模型名称、版本号、规模（参数/量化等，if disclosed） | GPT-4-0125, Claude 3 Opus, LLaMA-2-70B |
| **Dataset split / version** | 使用的数据集切分（test / dev）、版本号；是否可能 contamination | MMLU test split v2, GSM8K test set |
| **Prompt / eval protocol** | 评估协议 | zero-shot / few-shot / CoT / tool-use / human judge / LLM judge |
| **Metric** | 具体指标定义 | accuracy, pass@1, pass@k, F1, Elo score |
| **Baseline / no-input control** | 基线控制（no-image / text-only / random / human baseline） | text-only baseline: X%, random chance: Y% |
| **Source role** | 数据来源角色 | official report / benchmark paper / 第三方复现 / survey aggregation |
| **Comparability caveat** | 是否可横向比较，存在哪些限制 | 不同 prompt 设定不可直接横比；不同 dataset split 不可比 |

**Academic 侧重点与 technical-deep-dive 的区别**：
- academic-review 关注**方法学可比性**（prompt 设定、数据集切分、评估协议是否一致），而非硬件或部署环境
- technical-deep-dive 关注**工程可比性**（硬件、环境、延迟分布、并发数），此类字段在 academic-review 中通常不适用

### Cross-source comparability

When benchmark numbers from different papers, model reports, or third-party evaluations appear in the same comparison, the report must explicitly note comparability boundaries:

- **Different prompt protocols**（如 zero-shot vs. 5-shot CoT）cannot be compared without caveat
- **Different dataset splits**（如不同 MMLU subsets）may not be comparable
- **Different evaluation implementations**（如 original vs. third-party re-evaluation）should be noted
- **Contamination / leakage risk** for commonly used benchmarks should be discussed when material to the comparison

Silently juxtaposing numbers from incompatible contexts creates a false appearance of comparability.

### Graceful degradation

If a required field cannot be populated because the original source did not report it, the report should:

1. note the gap explicitly（"该论文未公开数据集切分信息"）
2. assess whether the missing information could materially change the comparison conclusion
3. adjust confidence or claim strength accordingly

When disclosure is incomplete for a load-bearing comparison:
- **只能给方向性判断**，不能给强排名
- **不能把单一 benchmark 上升为 general intelligence / planning ability** 等泛化能力结论
- 必须在 conclusion 中说明可比性限制

### Conclusion strength binding

Benchmark 可比性完整性直接约束结论强度：

| Schema 完整性 | 允许的结论强度 | 禁止的结论 |
|---------------|---------------|-----------|
| **完整**（所有 load-bearing 字段均已披露） | 可以给出条件性排名或能力判断 | 单一 benchmark → 通用智能 |
| **部分缺失**（1-2 个 load-bearing 字段不可用） | 仅方向性判断 | 强排名、泛化能力声称 |
| **严重缺失**（3+ 字段不可用或无 dataset/protocol 信息） | 仅可报告来源各自分数，不得放在同一比较表中 | 任何比较性结论 |

**示例**：若两个模型的 GSM8K 分数被并列比较，但其中一个来源未披露使用的 prompt 设定（zero-shot vs. few-shot CoT）和数据集版本——即使只有 2 个字段缺失（prompt protocol + dataset version），但缺少的字段直接影响可比性核心（无 protocol = 无法判断是否在同一条件下比较），因此应视为"严重缺失"而非"部分缺失"。此判断标准为：当缺失字段直接影响可比性核心条件（protocol、dataset split、metric definition）时，即使数量未达到 3+，也应视为严重缺失。
...
- `references/source-traceability-and-claim-citation.md` — 用于论文级（而非 benchmark 级）的 source traceability
...
### For field progress analysis (standard survey — 默认输出)
