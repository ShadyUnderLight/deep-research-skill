...
- **"系统评述""系统化综述""systematic review"**：暗示检索过程符合结构化标准（如 PRISMA 简化版）。只有报告完整文档化检索方法（数据库列表、检索日期、Boolean query、纳入/排除标准、screening counts）时方可使用。
- **替代措辞**：如果方法学达不到上述标准，应使用与透明度匹配的更精确措辞——

| 实际透明度 | 推荐措辞 |
|-----------|---------|
| 完整检索文档（数据库 + 检索式 + 筛选计数 + 日期） | 系统评述 / systematic review |
...
以下情况通过措辞纪律检查：
- ✅ 使用"文献回顾"措辞且检索步骤透明 → 通过
- ✅ 使用"系统检索"措辞且列明数据库、检索词、日期 → 通过
- ✅ 符合最低筛选计数标准的"有限范围系统化综述"且检索文档完整 → 通过
