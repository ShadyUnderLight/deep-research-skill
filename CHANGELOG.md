...
- `references/routes/` (#380): 12 generated route cards (11 specialized + shared-workflow), each linking to its full `ROUTING-MATRIX.md` contract, sibling cards, primary reads and checklist files.
- `tests/test_route_cards.py` (#380): 16 contract tests — generator determinism/idempotence, one card per manifest route with no orphans, generated-view declarations, card sections mirroring manifest fields (trigger / do-not-use / artifact contract / audits / disciplines / failure signs / hard-fail source), sibling-card links, link validity, and drift detection (missing / stale cards block `validate_route_manifest.py` with exit 2).
...
- `SKILL.md` (#380): routing rule is now progressive-disclosure — `references/route-index.md` → the route's card in `references/routes/` → `ROUTING-MATRIX.md` only when the card is not enough; Research Pack lifecycle points at the route card for artifact contract / required audits.
- `references/route-index.md` (#380): trigger table gains a `Card` column linking each route to its generated card (still ≤80 lines).
...
- `.github/workflows/ci.yml` (#380): `generate_route_cards.py --check` and `tests/test_route_cards.py` join the CLI gate; `validate-docs-structure.sh` Contract B now checks the technical-deep-dive route template for the terminology-boundary table and requires the core template to link it.
- `tests/test_issue_278/280/320/329_contracts.py`, `tests/test_capital_return_discipline.py` (#380): anchors moved from `references/report-template.md` to the route-specific templates that now own the content (listed-company / market-outlook / market-entry); cross-reference assertions accept either the core template or the route template.
