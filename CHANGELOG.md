...
- `scripts/run_forward_evals.py` (#393): forward runner records `schema_version` and `validators[]`, and fails closed on an unknown schema version, a missing/incomplete validator result, a forged validator id, missing evidence/provenance fields, or an illegal status — the recorded id set must exactly match the resolved route's canonical binding set instead of assuming Pass.
