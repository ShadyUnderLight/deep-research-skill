...
### Added
- `scripts/validate_figure_references.py` (#394): Mermaid fence state is now explicit — closed / mismatched / unclosed / invalid info string — using the shared fence semantics (`_fence_open_match` / `_fence_language` / `_fence_close_re`); unclosed or mis-closed (mismatched, shorter closer, Unicode-whitespace closer) Mermaid fences are BLOCKING (exit 2) and are NOT counted as legal figure entities, so a broken fence can no longer satisfy a 图N/Figure N reference; refs/captions inside invalid fences are ignored.
- `scripts/test_figure_reference_validator.py` (#394): 9 adversarial cases — unclosed Mermaid, fence closed only at EOF, backtick opener + tilde closer, shorter closer, NBSP trailing closer (all blocking), nested fence inside Mermaid, code-only caption/image, HTML-block pseudo-figure, and `mermaid-example` info string (all pass).
