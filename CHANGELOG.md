...
- `scripts/validate_research_pack.py`: added `## Stop condition` to required headings list; fenced code blocks are now stripped before heading checks (headings inside ``` blocks no longer count); empty required sections are detected and rejected.
- `.github/workflows/ci.yml`: added validator regression tests covering missing heading, code-block-only heading, empty section, and valid-pass scenarios.
