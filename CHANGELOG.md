...
### Added
- `tests/test_issue434_delivered_cross_artifact_binding.py` (#434): 25 delivered-gate cases — a real `audit_report --strict --require-contract --json` positive fixture, forged non-canonical secondary route plus forged `<route>-secondary-hard-fail`, unknown primary route (structured exit 2, no traceback), forged derived audit without contract declaration, audit route mismatch, phantom and fenced-hidden report locators, unresolvable/missing Pack `## Primary route`, duplicate/malformed contract blocks, post-audit tampering of the visible report route declaration (changed / deleted / duplicated status block), duplicate Pack `## Primary route` / `## Artifact id`, removed contract `artifact_id` and removed Pack `## Artifact id` (strict contract boundary / explicit Pack requirement), duplicate contract secondary route, tampered Pack `## Activation snapshot`, Run State `activation_reference` mismatch, an advisory-warning contract (`>2` secondary routes) that must stay deliverable, and the matching-chain positive controls.
