...
### Fixed
- `scripts/validate_research_run_state.py` (#434): the delivered gate no longer trusts a self-consistent audit JSON. The report contract now goes through the canonical `validate_contract()` boundary (single top-level ` ```contract ` block, canonical primary/secondary routes, no primary-as-secondary or duplicate secondaries, no unknown routes) before any expected audit id is derived, so a forged `<route>-secondary-hard-fail` for an undeclared route can no longer satisfy delivered; contract/Pack/audit routes are resolved canonically (no raw-string fallback) and an unresolvable Pack `## Primary route` fails closed instead of being silently skipped; unknown routes and registry/parse failures return structured delivered errors (exit code 2 with a JSON payload) instead of an unhandled `UnknownRouteError` traceback; report/Pack evidence locators are re-resolved against the canonical visible body, so phantom or fenced/HTML-hidden `report-section` / `report-table` / `pack-section` / `pack-table` evidence no longer passes on JSON self-attestation.
