...
### Fixed
- `scripts/validate_research_run_state.py` (#417): fail-closed the delivered / checkpoint / chain gates that previously opened — `--from/--to` into `delivered` now requires `--audit-result`, `--artifact` (Pack / `current_artifact_sha256`), and `--report` (`audit_report` `input_sha256`); those hashes stay distinct; delivery audit reuses canonical entry consistency and rejects missing/unknown status, missing `audit_id`, missing provenance `input_sha256`, and warning-less `conditional-pass`; `pending_decision` must be consumed before a phase advance; `--chain` requires the Pack-declared sidecar identity; `explicit_resume` rejects Track Handoff binding even when `artifact_id` aligns.
