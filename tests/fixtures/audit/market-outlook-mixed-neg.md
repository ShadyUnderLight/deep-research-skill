# Market Outlook 2027 with Secondary Route — Negative Fixture (issue #378)

The contract declares constrained-choice as a secondary route but no
`constrained-choice-secondary-hard-fail` entry exists. Both the contract
validation and the runtime audit must fail closed: secondary hard-fail
verification cannot be inferred from primary-route coverage.
