...
Read `references/parallel-research.md` when the task clearly benefits from multi-track work. Keep rate-limit risk in mind and prefer small batches over naive full parallelism; when `parallelization_decision` is `parallel`, validate every Track Handoff before merge with `python3 scripts/validate_track_handoff.py <handoff>.json` and bind it to this dispatch (`--expected-handoff-id` or `--expected-question` — `--expected-track-id` alone only proves the track name, not freshness). A failure is `HANDOFF_INCOMPLETE`: refuse to merge, never read it as "no evidence". Single-track research creates no handoffs.
