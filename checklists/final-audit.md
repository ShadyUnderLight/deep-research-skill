...
- [ ] for constrained-choice / shortlist / option-selection reports, the first-screen clarity includes the core judgment followed by decision scope (target reader, option universe, shortlist boundary, exclusions) before route metadata or background（另见 `checklists/option-selection-final-audit.md` §Decision Scope visibility）
...
- [ ] for academic / literature review tasks, coverage window is declared in the body text and conclusions reflect that window's temporal bounds; recent papers (<3 months before review date) have peer-review status backed by verification evidence (proceedings / OpenReview / publisher page / official accepted list)
- [ ] for academic / literature review tasks, benchmark comparability schema is applied when AI benchmark comparisons are involved: each load-bearing comparison discloses Benchmark / Task, Capability claim, Model / version, Dataset split / version, Prompt / eval protocol, Metric, Baseline / no-input control, Source role, and Comparability caveat per `references/academic-evidence-hierarchy.md` §Benchmark comparability for academic reviews; when disclosure is incomplete for load-bearing comparisons, conclusions are downgraded to directional judgment (not strong ranking or general intelligence claims)
...
- [ ] (technical-deep-dive 适用) technical report front page 不只展示 evidence legend / route metadata / 背景说明；它必须先展示技术判断、决策场景、关键风险和版本基线（见 `references/templates/technical-deep-dive-report.md` §Technical deep-dive opening）（非阻塞）
...
- [ ] figure reference completeness validated: run `python3 scripts/validate_figure_references.py <report.md>` — every "图X" / "Figure X" reference in body text must have a corresponding caption or figure entity; duplicate captions are blocked; Mermaid blocks without captions and figure numbering gaps are flagged as warnings. Note (issue #394): figure validation is an **independent advisory check** — it is NOT part of the unified `scripts/audit_report.py` audit registry, so running `audit_report.py --strict` does NOT cover figure references. Run this validator separately when figures are part of the report. Unclosed or mis-closed (mismatched / shorter / Unicode-whitespace) Mermaid fences are blocking (exit 2) and are not counted as legal figure entities.
