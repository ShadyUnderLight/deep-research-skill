...
- [ ] （阻断级）secondary route hard-fail verification may not be skipped: when a secondary route is declared, each of its hard-fail conditions must be verified independently. "Covered elsewhere without independent run" is not an acceptable verification reason — a secondary route's conditions are not exempted because content is already covered by the primary route. If a hard-fail condition is genuinely inapplicable to the task (structurally irrelevant rather than "already covered"), document the inapplicability reason per the existing exception above (line 12); do not skip it silently
